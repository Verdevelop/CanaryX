...
import { cnya_cand } from "./AllBonds";
...
export async function getMarketPrice({ networkID, provider }: IBaseAsyncThunk) {
  const cnya_cand_address = cnya_cand.getAddressForReserve(networkID);
  const pairContract = new ethers.Contract(cnya_cand_address, PairContract, provider);
...
export async function getTokenPrice(tokenId = "dozen-dao") {
...
export function getRebaseBlock(currentTime: number) {
  return currentTime + EPOCH_INTERVAL - (currentTime % EPOCH_INTERVAL);
}

export function secondsUntilBlock(startTime: number, endTime: number) {
  const secondsAway = endTime - startTime;
  return secondsAway;
}

export function prettyVestingPeriod(currentTime: number, vestingTime: number) {
  if (vestingTime === 0) {
    return "";
  }
