...
export const loadAppDetails = createAsyncThunk(
  ...
  `;
    ...
    const sHecMainContract = new ethers.Contract(addresses[networkID].SCNYA_ADDRESS as string, sHECv2, provider);
    const hecContract = new ethers.Contract(addresses[networkID].CNYA_ADDRESS as string, ierc20Abi, provider);
    const pdznBalance = await hecContract.balanceOf(addresses[networkID].STAKING_ADDRESS);
    const stakingTVL = marketPrice * pdznBalance / 1000000000;
    console.log("staking", marketPrice, pdznBalance)
    ...
    document.title = `DozenDAO - ${marketPriceString}`;
    if (!provider) {
 ...
 * - falls back to fetch marketPrice from cnya_cand contract
