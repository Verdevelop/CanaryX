...
export const changeApproval = createAsyncThunk(
  ...
  async ({ token, provider, address, networkID }: IChangeApprovalAsyncThunk, { dispatch }) => {
    ...
    const hecContract = new ethers.Contract(addresses[networkID].CNYA_ADDRESS as string, ierc20Abi, signer);
    const shecContract = new ethers.Contract(addresses[networkID].SCNYA_ADDRESS as string, ierc20Abi, signer);
