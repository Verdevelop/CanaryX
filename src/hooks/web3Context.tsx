...
export const Web3ContextProvider: React.FC<{ children: ReactElement }> = ({ children }) => {
  ...
  const [chainID, setChainID] = useState(19);
  const [vchainID, setVChain] = useState(19);
  ...
  const _checkNetwork = (otherChainID: number): Boolean => {
    if (chainID !== otherChainID) {
      ...
      if (otherChainID === 19) {
        ...
        otherChainID === 19 ? setUri(getMainnetURI()) : setUri(getTestnetURI());
        return true;
      }
      return false;
    }
  ...
  const connect = useCallback(async () => {
    ...
    if (!validNetwork) {
      console.error("Wrong network, please switch to songbird");
